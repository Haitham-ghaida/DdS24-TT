...
def postprocessing(final_res,insitu):
    ...
    column_names = ['flow name','flow unit','flow quantity','year','process','region']
    ...
    if insitu.empty == False:
        ...
        column_names = ['flow name', 'flow unit', 'year', 'process','region']
...
def impact_calculations(final_res,traci_lci_filename):    
    ...
    df_lcia = df_lcia.groupby(['year', 'process', 'region', 'impacts'])['impact'].agg('sum').reset_index()  
