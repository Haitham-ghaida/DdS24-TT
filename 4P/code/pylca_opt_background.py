...
def model_celavi_lci_background(f_d, yr, stage,
    ...
    process_emissions = pd.read_csv("./input/core_files/process_emissions_corr.csv")
    #Creating the technology matrix for performing LCA caluclations
    tech_matrix = pd.read_csv('./input/core_files/tech_matrix_corr.csv', index_col ='conjoined_flownames')
    ...
    def runner(tech_matrix, F,i,j,final_demand_scaler):
        ...
        if not res.empty:
          ...
          print(f"Calculation failed for background at {j} in {i}") 
          pass
    ...
    process_adder = pd.read_csv("./input/core_files/process_names_adder.csv")
    ...
    f_d.to_csv('./output/intermediate_demand_foreground_uslci_data.csv', mode = 'a')
