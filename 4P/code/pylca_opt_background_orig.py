...
def model_celavi_lci_background(f_d, yr, stage, uslci_filename,
    ...
    #process_df = pd.read_csv("./input/core_files/process_df_old.csv")
    #process_product = pd.read_csv("./input/core_files/process_product_old.csv")
    #process_emissions = pd.read_csv("./input/core_files/process_emissions_old.csv")
   
    process_df.to_csv('./output/process_df_new.csv')
    process_product.to_csv('./output/process_product_new.csv')
    process_emissions.to_csv('./output/process_emissions_new.csv')
    ...
    def runner(tech_matrix, F,i,j,final_demand_scaler):
        ...
        if not res.empty:
          ...
          print(f"optimization pylca-opt-background emission failed at {j} in {i}") 
          pass
    ...
    f_d['flow name'] = f_d['flow name'] +'@' + f_d['process']
    ...
    final_dem.to_csv('./output/finaldemand.csv')
    ...
    if chksum == 0:
        ...
        final_dem.to_csv('./output/Final_demand_check_file.csv')
        sys.exit(1)
    ...
    res2 = runner(tech_matrix,F,yr,stage,final_demand_scaler)
